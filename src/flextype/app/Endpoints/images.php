...
$flextype->get('/api/images/{path:.+}', function (Request $request, Response $response, $args) use ($container, $api_errors) {
    ...
    if (! isset($query['token'])) {
        return $response->withStatus($api_errors['0400']['http_status_code'])
            ...
            ->write($container->json->encode($api_errors['0400']));
    }

    // Set variables
    $token = $query['token'];

    if ($container['registry']->get('flextype.settings.api.images.enabled')) {
        ...
        if (validate_images_token($token)) {
            ...
            if ($delivery_images_token_file_data = $container['yaml']->decode(Filesystem::read($delivery_images_token_file_path))) {
            ...
            ->write($container->json->encode($api_errors['0003']));
                ...
                Filesystem::write($delivery_images_token_file_path, $container['yaml']->encode(array_replace_recursive($delivery_images_token_file_data, ['calls' => $delivery_images_token_file_data['calls'] + 1])));

                if (Filesystem::has(PATH['project'] . '/uploads/entries/' . $args['path'])) {
                    return $container['images']->getImageResponse($args['path'], $_GET);
                }

                return $response
                    ->withStatus($api_errors['0402']['http_status_code'])
            ->withHeader('Content-Type', 'application/json;charset=' . $container->registry->get('flextype.settings.charset'))
            ->write($container->json->encode($api_errors['0402']));
            }
