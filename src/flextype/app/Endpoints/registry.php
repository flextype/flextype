...
$flextype->get('/api/registry', function (Request $request, Response $response) use ($container, $api_errors) {
    ...
    if (! isset($query['id']) || ! isset($query['token'])) {
        return $response->withStatus($api_errors['0300']['http_status_code'])
            ...
            ->write($container->json->encode($api_errors['0300']));
    }
    ...
    if ($container['registry']->get('flextype.settings.api.registry.enabled')) {
        // Validate  token
        if (validate_registry_token($token)) {
            $registry_token_file_path = PATH['project'] . '/tokens/registry/' . $token . '/token.yaml';

            // Set  token file
            if ($registry_token_file_data = $container['yaml']->decode(Filesystem::read($registry_token_file_path))) {
            ...
            ->write($container->json->encode($api_errors['0003']));
                ...
                if ($container['registry']->has($id)) {
                    ...
                    $response_data['data']['value'] = $container['registry']->get($id);
                ...
                Filesystem::write($registry_token_file_path, $container['yaml']->encode(array_replace_recursive($registry_token_file_data, ['calls' => $registry_token_file_data['calls'] + 1])));
            ...
            ->withHeader('Content-Type', 'application/json;charset=' . $container->registry->get('flextype.settings.charset'))
            ->write($container->json->encode($api_errors['0302']));
                }
