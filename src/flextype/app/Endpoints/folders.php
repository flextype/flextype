...
$flextype->get('/api/folders', function (Request $request, Response $response) use ($container, $api_errors) {
    ...
    if (! isset($query['path']) || ! isset($query['token'])) {
        return $response->withStatus($api_errors['0600']['http_status_code'])
                        ...
                        ->write($container->json->encode($api_errors['0600']));
    }
    ...
    if ($container['registry']->get('flextype.settings.api.folders.enabled')) {
        // Validate delivery token
        if (validate_folders_token($token)) {
            $folders_token_file_path = PATH['project'] . '/tokens/folders/' . $token . '/token.yaml';

            // Set delivery token file
            if ($folders_token_file_data = $container['yaml']->decode(Filesystem::read($folders_token_file_path))) {
                ...
                if ($collection) {
                    $folders = $container['media_folders']->fetchCollection($path);
                } else {
                    $folders = $container['media_folders']->fetchSingle($path);
                }
                ...
                Filesystem::write($folders_token_file_path, $container['yaml']->encode(array_replace_recursive($folders_token_file_data, ['calls' => $folders_token_file_data['calls'] + 1])));
                ...
                if ($response_code === 404) {
                    ...
                    return $response
                           ...
                           ->withHeader('Content-Type', 'application/json;charset=' . $container->registry->get('flextype.settings.charset'))
                           ->write($container->json->encode($api_errors['0602']));
                }
...
$flextype->post('/api/folders', function (Request $request, Response $response) use ($container, $api_errors) {
    ...
    if (! isset($post_data['token']) || ! isset($post_data['access_token']) || ! isset($post_data['path'])) {
        return $response->withStatus($api_errors['0601']['http_status_code'])
            ->withHeader('Content-Type', 'application/json;charset=' . $container->registry->get('flextype.settings.charset'))
            ->write($container->json->encode($api_errors['0601']));
    }
        ...
        if (validate_folders_token($token) && validate_access_token($access_token)) {
            ...
            if (($folders_token_file_data = $container['yaml']->decode(Filesystem::read($folders_token_file_path))) &&
              ($access_token_file_data = $container['yaml']->decode(Filesystem::read($access_token_file_path)))) {
                ...
                // Create folder
                $create_folder = $container['media_folders']->create($path);

                $response_data = [];

                if ($create_folder) {
                    $response_data['data'] = $container['media_folders']->fetch($path);
                }
    ...
    if ($container['registry']->get('flextype.settings.api.folders.enabled')) {
        ...
        if (validate_folders_token($token) && validate_access_token($access_token)) {
            ...
            ->write($container->json->encode($api_errors['0601']));
                ...
                // Copy folder
                $copy_folder = $container['media_folders']->copy($path, $new_path);

                $response_data = [];

                if ($copy_folder) {
                    $response_data['data'] = $container['media_folders']->fetch($new_path);
                } else {
        ...
        if (validate_folders_token($token) && validate_access_token($access_token)) {
            ...
            if (($folders_token_file_data = $container['yaml']->decode(Filesystem::read($folders_token_file_path))) &&
              ($access_token_file_data = $container['yaml']->decode(Filesystem::read($access_token_file_path)))) {
                ...
                // Rename folder
                $rename_folder = $container['media_folders']->rename($path, $new_path);
        ...
        if (validate_folders_token($token) && validate_access_token($access_token)) {
            ...
            if (($folders_token_file_data = $container['yaml']->decode(Filesystem::read($folders_token_file_path))) &&
              ($access_token_file_data = $container['yaml']->decode(Filesystem::read($access_token_file_path)))) {
                ...
                // Delete folder
                $delete_folder = $container['media_folders']->delete($path);
                ...
                return $response
                     ->withStatus($response_code)
            ->withHeader('Content-Type', 'application/json;charset=' . $container->registry->get('flextype.settings.charset'))
            ->write($container->json->encode($delete_folder));
            }
