...
function processParsersField(): void
{
    ...
    if (flextype('entries')->getStorage('fetch_single.data.parsers') != null) {
        foreach (flextype('entries')->getStorage('fetch_single.data.parsers') as $parserName => $parserData) {
            if (in_array($parserName, ['markdown', 'shortcode'])) {
                if (flextype('entries')->getStorage('fetch_single.data.parsers.'.$parserName.'.enabled') === true) {
                    if (flextype('entries')->getStorage('fetch_single.data.parsers.'.$parserName.'.fields') != null) {
                        if (is_array(flextype('entries')->getStorage('fetch_single.data.parsers.'.$parserName.'.fields'))) {
                            foreach (flextype('entries')->getStorage('fetch_single.data.parsers.'.$parserName.'.fields') as $field) {
